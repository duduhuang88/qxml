package com.qxml.qxml_androidx.gen.spinner;
...
public class SpinnerHelper {
    ...
    private static final Constructor<?> CONSTRUCTOR_DialogPopup = ReflectUtils.getDeclaredConstructorOrNull(classDialogPopup, Spinner.class);
    ...
    public static void setAttr(Spinner spinner, Context context, int dropDownSelector) {
        try {
            if (mPopupField != null) {
                ...
                } else if (mPopup instanceof androidx.appcompat.widget.ListPopupWindow) {
                    ((androidx.appcompat.widget.ListPopupWindow) mPopup).setListSelector(com.qxml.tools.DrawableTools.getDrawable(context, context.getResources(), dropDownSelector));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
