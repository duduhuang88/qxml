...
public class ResConfigFlags {
	...
	public ResConfigFlags(short mcc, short mnc, char[] language, char[] region, byte orientation, byte touchscreen,
		...
		if (orientation < 0 || orientation > 3) {
			LogUtil.pl("Invalid orientation value: " + orientation);
			orientation = 0;
			isInvalid = true;
		}
		if (touchscreen < 0 || touchscreen > 3) {
			LogUtil.pl("Invalid touchscreen value: " + touchscreen);
			touchscreen = 0;
			isInvalid = true;
		}
		if (density < -1) {
			LogUtil.pl("Invalid density value: " + density);
			density = 0;
			isInvalid = true;
		}
		if (keyboard < 0 || keyboard > 3) {
			LogUtil.pl("Invalid keyboard value: " + keyboard);
			keyboard = 0;
			isInvalid = true;
		}
		if (navigation < 0 || navigation > 4) {
			LogUtil.pl("Invalid navigation value: " + navigation);
			navigation = 0;
			isInvalid = true;
		}
