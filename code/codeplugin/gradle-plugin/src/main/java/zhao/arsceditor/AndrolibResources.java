...
//from https://github.com/ha1vk/ArscEditor. thanks a lot
final public class AndrolibResources {
	...
	public void decodeARSC(ResTable resTable, ARSCCallBack callback) throws IOException {
		...
		for (ResPackage pkg : resTable.listMainPackages()) {
			LogUtil.pl("Decoding values */* XMLs...");
	...
	private void generateValuesFile(ResValuesFile valuesFile, ARSCCallBack callback) throws IOException {
		for (ResResource res : valuesFile.listResources()) {
			...
			if (value instanceof ResFileValue) {
				if ("layout".equals(res.getResSpec().getType().getName())) {
					((ResFileValue) value).getResValues(callback, res);
				}
			}
		}
	}
	...
	public ResPackage loadMainPkg(ResTable resTable, InputStream ARSCStream) throws IOException {
		LogUtil.pl("Loading resource table...");
		...
		case 2:
			if (pkgs[0].getName().equals("android")) {
				LogUtil.pl("Skipping \"android\" package group");
				pkg = pkgs[1];
			} else if (pkgs[0].getName().equals("com.htc")) {
				LogUtil.pl("Skipping \"htc\" package group");
				pkg = pkgs[1];
			}
			break;
		default:
